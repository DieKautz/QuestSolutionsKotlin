...
fun main() {
    ...
    val issuerAccountKeys = KeyPair.fromSecretSeed(Keys.set1helperKey)

    val txBuilder = Transaction.Builder(questAccount, Network.TESTNET)
        .setBaseFee(FeeBumpTransaction.MIN_BASE_FEE)
        .setTimeout(180)

    val myAsset = Asset.createNonNativeAsset("COOL", issuerAccountKeys.accountId)

    txBuilder.addOperation(
        ManageSellOfferOperation.Builder(myAsset, AssetTypeNative(), "1", "0.0001").build()
    )
